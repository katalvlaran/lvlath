...
Thanks a lot for reaching out and for the context about OTO -  the idea of a “script scheduler with argument constraints” is very practical, and I can immediately see why you ended up thinking about graphs.
...
This maps very naturally to a small directed graph plus a symmetric conflict relation.

In my library `lvlath` (github.com/katalvlaran/lvlath, current `main` branch), I would model it as:
- A directed, unweighted graph G over your argument IDs using the `core` package:
  - each argument is a vertex;
  - for every “A requires B” I add a directed edge A → B with weight 0.
- A separate symmetric map for conflicts (much cheaper and clearer than trying to encode conflicts into the same graph).

Then the algorithm is straightforward and stays small:

1) Schema validation at startup

Before executing anything, I would validate the constraint schema itself once at startup:
- Use `dfs.TopologicalSort` from `lvlath/dfs` on the dependency graph to detect cycles.
  A cycle usually means the schema is misconfigured (arguments that require each other) or that a group should be treated as a single higher-level option. For a CLI-style API I would usually treat this as a hard error and fail fast.
- For each conflict pair (A, B), I would use `bfs.BFS` from `lvlath/bfs` to check reachability:
  - run BFS from A and see if B is reachable via “requires” edges;
  - run BFS from B and see if A is reachable.
  If either direction is reachable, the schema says “A and B must conflict”, but the dependencies say “picking one forces the other”. That’s a structural contradiction, and I would reject the schema up front instead of discovering it at runtime.

This phase runs once and keeps your rules self-consistent.

2) Validating a concrete selection

For each user selection S:
- Start with a set `need = S`.
- For every argument `a` in `S`, run `bfs.BFS` starting from `a` on the dependency graph.
  - The BFS result gives you all vertices reachable from `a`, i.e. direct and transitive dependencies.
  - Add all these vertices into `need`. After this, `need` contains the original selection plus all required arguments.

Then check conflicts:
- For every conflict pair (A, B), if both A and B are present in `need`, reject this selection.
- Optionally, you can run another BFS from A and reconstruct the shortest path to B using the BFS parent map to generate a very clear explanation such as:
  “`--cron` implies `--timezone` → `--lock-file` → `--force`, which conflicts with `--dry-run`”.

For the scale you described (not huge numbers of arguments), the complexity is basically O(V + E) per BFS, and the overhead is negligible compared to I/O or process execution.

Why bother with a graph if the number of arguments is small?

For me the main value is not raw performance but clarity and future-proofing:
- The rules become an explicit dependency graph instead of scattered if/else logic.
- Transitive dependencies work “for free” because BFS/DFS naturally traverse chains like A → B → C.
- The same structure can be reused for:
  - explaining *why* a particular argument was auto-added,
  - computing a safe execution order via `dfs.TopologicalSort` on the induced subgraph of all selected arguments,
  - detecting “dead” options that are never reachable from any public entry point.

In other words, even with a small argument set, a tiny deterministic graph layer tends to pay off in correctness and in how easy it is to extend the rules over time.

How lvlath fits into this

Right now the `main` branch already reflects the architecture I am preparing to tag as v1.0.0. The most relevant packages for your use case are:
- `core` -  a deterministic, thread-safe `Graph` type with options for directed/undirected, weighted/unweighted, multi-edges, self-loops, and induced subgraphs.
- `bfs` — breadth-first search with a stable visit order, depth map, and parent tree (perfect for reachability and “why” explanations).
- `dfs` — depth-first traversal with `DetectCycles` and `TopologicalSort`, which is exactly what I use for dependency graphs and schedulers.

On top of that there are other pieces (max-flow, TSP, DTW, matrix utilities), but for OTO the combination `core + bfs + dfs` is already enough to implement a robust argument-constraint engine.

A few extra ideas you could explore with lvlath later:
- Generate human-readable explanations: reconstruct shortest paths between conflicting arguments and show “chains of implications” to the user.
- Visualize your constraints: export the `core.Graph` to a DOT/GraphML representation for documentation or debugging.
- Group rules: treat “exactly one of {--now, --at, --cron}” as a small motif in the graph and validate such groups structurally.

I am currently preparing more focused documentation and examples around these patterns, and an “argument-constraints / scheduler” scenario is one of the examples I plan to include. Master already contains the architecture and APIs I intend to release as v1.0.0, with more examples and docs coming with the tag.

If you have any concrete rule patterns (e.g. “exactly one of this group”, “at most one of that group”) or edge cases that are important for OTO, I’d be very interested to hear them — this kind of real-world feedback is exactly what helps me refine both the library and the examples.
